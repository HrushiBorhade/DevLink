...
const HeroImage = () => {
  ...
  return (
    <div ref={ref} className="[perspective:2000px]   mt-24">
      <div
        className={cn(
      ...
      >
        <div className="absolute top-0 left-0 z-20 w-full h-full">
          {lines.map((line) => (
            <span
              ...
              className={cn(
                "absolute top-0 block h-[1px] w-[100px] bg-glow-lines",
                line.direction === "to left" &&
                  `left-0 h-[1px] w-[calc(var(--size)*0.4rem)] animate-glow-line-horizontal md:w-[calc(var(--size)*0.7rem)]`,
                line.direction === "to top" &&
                  `right-0 h-[calc(var(--size)*0.4rem)] w-[1px] animate-glow-line-vertical md:h-[calc(var(--size)*0.7rem)]`
              )}
            />
          ))}
        </div>
        <svg
          className={cn(
            ...
            "[&_path]:stroke-white [&_path]:[strokeOpacity:0.6] [&_path]:[stroke-dasharray:1] [&_path]:[stroke-dashoffset:1]",
