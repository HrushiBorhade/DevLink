...
export async function POST(req: Request) {
  try {
    ...
    if (communityExists) {
      return new Response(`Community with ${name} already exists`, {
        status: 409,
      });
    }

    //create community and associate it with user
    ...
    return new Response("Failed to create Community :(", { status: 500 });
  }
}
